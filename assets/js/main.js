    ...
    if (benefit) {
      ...
      if (statusBadge) {
        ...
        statusBadge.className = `badge ${
          benefit.status === "active" ? "bg-success" : "bg-warning"
        }`;
      }
    }
  });
...
function updateQRUserInfo(userData) {
  ...
  if (qrUserLocation)
    qrUserLocation.textContent = `From: ${userData.homeState}, Currently in: ${userData.currentState}`;
...
function generateQRCode(userData) {
  ...
  if (qrCodeDiv) {
    ...
    try {
      // Use our simple QR library
      console.log("Creating QR code with simple library...");
      ...
      // Add visual indicator for unique QR after a short delay
      setTimeout(() => {
      ...
      qrCodeDiv.innerHTML = `
        <div class="bg-light p-4 rounded text-center">
          ...
          <p class="text-muted">QR Code for ${userData.name}</p>
...
function generateChecksum(userData) {
  const dataString =
    userData.id + userData.name + userData.aadhaar + userData.phone;
  let checksum = 0;
...
function applyForBenefit(benefitType) {
  ...
  setTimeout(() => {
    showAlert(
      `Application for ${benefitType} benefits submitted successfully!`,
      "success"
    );
  }, 1500);
}
...
function reportIssue(benefitType) {
  ...
  setTimeout(() => {
    showAlert(
      "Issue reported successfully. Reference ID: INC123456",
      "success"
    );
  }, 1000);
}
...
function transferBenefits(benefitType) {
  ...
  setTimeout(() => {
    showAlert(
      "Transfer process initiated. You will receive updates via SMS.",
      "success"
    );
  }, 1500);
}
...
function applyForLoan() {
  ...
  setTimeout(() => {
    showAlert(
      "Loan application submitted. You are eligible for ₹50,000",
      "success"
    );
  }, 1500);
}
...
function initiateTransfer() {
  ...
  setTimeout(() => {
    showAlert(
      "Transfer process initiated. Required documents will be sent via email.",
      "success"
    );
  }, 1500);
}
                ...
                <body>
                    <div class="qr-container">
                        ...
                        <div class="user-info">
                            ...
                            ID: ${userData.id}<br>
                            From: ${userData.homeState} → Currently in: ${
...
function validateQRCode(qrData) {
  try {
    ...
    const userData =
      users[Object.keys(users).find((key) => users[key].id === data.migrantId)];
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <ul class="list-unstyled">
                                ...
                                <li><strong>Expiry:</strong> ${new Date(
                                  data.expiryDate
                                ).toLocaleDateString()}</li>
                        ...
                        <div class="col-md-3 text-center">
                            <div class="border rounded p-2">
                                ...
                                <div><span class="badge bg-${
                                  data.benefits.food.status === "active"
                                    ? "success"
                                    : "warning"
                                }">${data.benefits.food.status}</span></div>
                            </div>
                        </div>
                        <div class="col-md-3 text-center">
                            <div class="border rounded p-2">
                                ...
                                <div><span class="badge bg-${
                                  data.benefits.health.status === "active"
                                    ? "success"
                                    : "warning"
                                }">${data.benefits.health.status}</span></div>
                            </div>
                        </div>
                        <div class="col-md-3 text-center">
                            <div class="border rounded p-2">
                                ...
                                <div><span class="badge bg-${
                                  data.benefits.education.status === "active"
                                    ? "success"
                                    : "warning"
                                }">${
    data.benefits.education.status
  }</span></div>
                            </div>
                        </div>
                        <div class="col-md-3 text-center">
                            <div class="border rounded p-2">
                                ...
                                <div><span class="badge bg-${
                                  data.benefits.finance.status === "active"
                                    ? "success"
                                    : "warning"
                                }">${data.benefits.finance.status}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
...
function showAlert(message, type = "info") {
  ...
  alertDiv.style.cssText =
    "top: 20px; right: 20px; z-index: 1060; max-width: 400px;";
  alertDiv.innerHTML = `
        ${message}
